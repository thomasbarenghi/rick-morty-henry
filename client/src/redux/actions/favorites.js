...
export const favoriteCharacter = (character) => (dispatch, getState) => {
    ...
    const config = { headers: { Authorization: `Bearer ${token}`, userId: userId } };
    const state = getState();
    const isAlreadyFavorite = state.personajes.favoritos.find((c) => c.id === character.id);

    if (isAlreadyFavorite) {
        axios.delete(`${SERVER_URL}/client/favorites/${character.id}`, config)
            .then((res) => { dispatch(getFavorites()); })
            .catch((err) => console.error(err));
    }

    else {
        axios.post(`${SERVER_URL}/client/favorites`, { defaultCharacterId: character.id }, config)
            .then((res) => { dispatch(getFavorites()); })
            .catch((err) => console.error(err));
    }
